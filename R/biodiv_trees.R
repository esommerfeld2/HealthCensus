...
#'
#' @description Calculates tree biodiversity by quadrat using data on the species composition per quadrat. Merges the calculated biodiversity index values back into the full dataset. Specifically takes abundance of species at the quadrat level to compute biodiversity index values (Shannon index, effective number of species, species richness, maximum possible diversity, Pielou's evenness J) and appends these index values to each tree in the given quadrat. It then computes summaries of indices of tree crown health and mortality rate, allowing easy visualization of the relationship between a tree's biodiversity index and its mortality.
